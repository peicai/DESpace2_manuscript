...
run_spatialLIBD_pairwise <- function(sce.combined,
                                   output_path,
  ...
  save(results_res, file = paste0(output_path, "spatialLIBD_pairwise_", 
                                   cluster_method, ".rda"))
}
run_spatialLIBD_enrichment <- function(sce.combined,
                                   output_path,
                                   cluster_method = "BayesSpace",
                                   cluster_col = "matched_cluster"){
  ## Add gene-level information
  ...
  for (each_layer in levels(sce.combined$Cluster)){
    ...
    results_res[[each_layer]] <- results_enrichment
  }
  save(results_res, file = paste0(output_path, "spatialLIBD_enrichment_", 
                                  cluster_method, ".rda"))
}
run_spatialLIBD_anova <- function(sce.combined,
                                  output_path,
                                  cluster_method = "BayesSpace",
                                  cluster_col = "matched_cluster"){
  ## Add gene-level information
  ...
  for (each_layer in levels(sce.combined$Cluster)){
    ...
    results_res[[each_layer]] <- results_anova
  }
  save(results_res, file = paste0(output_path, "spatialLIBD_anova_", 
                                  cluster_method, ".rda"))
}
...
run_seurat_PseudoBulk_FindMarkers <- function(sce.combined,
  ...
  grp1 <- paste0("Condition1_",levels(sce.combined$Cluster))
  grp2 <- paste0("Condition2_",levels(sce.combined$Cluster))
  results_res <- list()
...
run_seurat_FindAllMarkers <- function(sce.combined,
  ...
  for (each_layer in levels(factor(sce.combined$Cluster))){
    ...
    # find markers for every condition compared to all remaining cells
    res1 <- Seurat::FindAllMarkers(seurat_norm,
                                   grouping.var = "Sample",
                                   logfc.threshold = -Inf,
                                   min.pct = 0,
                                   test.use = "wilcox",
    ...
    results_res1[[each_layer]] <- res1
  }
  
  save(results_res1, file = paste0(output_path, "seurat_FindAllMarkers_", 
                                   cluster_method, ".rda"))
