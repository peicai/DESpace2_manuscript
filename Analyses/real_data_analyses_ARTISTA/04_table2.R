...
############################ gene lists from https://www.proteinatlas.org/search ################################
# list files in a folder (healing.json and regeneration.json)
gene_folder = "./data/ARTISTA/gene_list/"
...
for (method_name in names(comparison_tables_with_row_names)) {
  ...
  regen_top_genes <- as.numeric(top_genes_row[3])
  heal_top_genes <- as.numeric(top_genes_row[2])
  
  regen_avg_rank  <- as.numeric(avg_rank_row[3])
  heal_avg_rank  <- as.numeric(avg_rank_row[2])
  
  # Combine into a data frame row
  summary_list[[method_name]] <- data.frame(
    Method = method_name,
    Regeneration_TopGenes = regen_top_genes,
    Healing_TopGenes = heal_top_genes,
    Total = regen_top_genes + heal_top_genes,
    Regeneration_AvgRank = regen_avg_rank,
    Healing_AvgRank = heal_avg_rank,
