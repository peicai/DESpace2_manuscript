...
path <- "./"
### 2-group comparison
load("DESpace_global_test_2vs20.rda")
gene_order <- res_edgeR$gene_results$gene_id; rm(res_edgeR)
generalize_transformation(
  dir = path, 
  file_prefix = "seurat_FindMarkers_Banksy_2vs20"
)

generalize_transformation(
  dir = path, 
  file_prefix = "scran_findMarkers_Banksy_2vs20", 
  pval_col = "p.value", 
  padj_col = "FDR"
)
...
generalize_transformation(
  dir = path, 
  file_prefix = "seurat_PseudoBulk_FindMarkers_Banksy_2vs20"
)

generalize_transformation(
  dir = path, 
  file_prefix = "spatialLIBD_pairwise_Banksy_2vs20",
  pval_col = "p_value_Condition1-Condition2",
  padj_col = "fdr_Condition1-Condition2"
)
generalize_transformation(
  dir = path, 
  file_prefix = "DESpace_individual_test_2vs20", 
  pval_col = "PValue", 
  padj_col = "FDR"
)
#### 5 groups #####
# NOTE: 
# seurat FindMarkers would do pairwise comparison: condition1 vs. all other conditions
# spatialLIBD pairwise and enrichment do pairwise comparison for each pair of conditions
load("DESpace_global_test_all5.rda")
gene_order <- res_edgeR$gene_results$gene_id; rm(res_edgeR)
generalize_transformation(
  dir = path, 
  file_prefix = "scran_findMarkers_Banksy_all5", 
  pval_col = "p.value", 
  padj_col = "FDR"
)
generalize_transformation(
  dir = path, 
  file_prefix = "spatialLIBD_anova_Banksy_all5",
  pval_col = "p_value_",
  padj_col = "fdr_"
)
generalize_transformation(
  dir = path, 
  file_prefix = "DESpace_individual_test_all5", 
  pval_col = "PValue", 
  padj_col = "FDR"
)
...
process_method_results <- function(method_name_file, method_name_script, dir, 
  ...
  filter_and_format <- function(df, gene_order, method_name_script, method_name_file, domain_name, pval_col_name, padj_col_name) {
    if(method_name_file == "seurat_FindMarkers_Banksy") {
      df <- df[df$cluster == "Condition1", ]
    }
...
method_mapping <- list(
  "DESpace_individual_test_2vs20" = "DESpace",
  "DESpace_individual_test_all5" = "DESpace",
  "seurat_FindMarkers_Banksy_2vs20" = "FindMarkers",
  "scran_findMarkers_Banksy_2vs20" = "findMarkers",
  "seurat_PseudoBulk_FindMarkers_Banksy_2vs20" = "pseudo.bulk_FindMarkers",
  "spatialLIBD_pairwise_Banksy_2vs20" = "spatialLIBD_pairwise"
  
)

# Define column names for pval and padj
pval_col_names <- c("PValue","PValue",
                    "p_val", 
                    "p.value", 
                    "p_val", 
                    "p_value_Condition1-Condition2")
padj_col_names <- c("FDR","FDR","p_val_adj",
                    "FDR", "p_val_adj", 
                    "fdr_Condition1-Condition2")
logFC_col_names <- c("logFC","logCPM","avg_log2FC",
                     "summary.logFC","avg_log2FC", 
                     "logFC_Condition1-Condition2")

data_names <- c("res_edgeR","res_edgeR","results_res1", 
                rep("results_res", 3))
