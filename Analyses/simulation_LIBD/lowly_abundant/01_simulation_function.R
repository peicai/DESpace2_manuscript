...
process_subset <- function(subset_sce) {
  ...
  cell_meta <- as.data.table(
    colData(subset_sce)[, c("imagerow", "imagecol", "barcode", "layer_guess_reordered_droplevel",
...
CombineSimulatedObject = function(processed_sce_list,
  ...
  results_all <- lapply(seq_along(gene_list), function(gene_ind) {
    ...
    process_gene(gene, gene_ind, gene_list, scenario, spatial_probs, para, 
                 pattern_name, processed_sce_list, region_gene, cluster_col)
  })
  
  ################# CONDITION 1 ############
  sce_object1 <- create_sce_object(1, gene_list, results_all, processed_sce_list)
  
  ################# CONDITION 2 ############
  sce_object2 <- create_sce_object(2, gene_list, results_all, processed_sce_list)
  
  return(list(sce_object1, sce_object2))
}
