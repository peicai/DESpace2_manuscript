...
legend <- readRDS(paste0(path_fig, "legend_methods.rds"))
FDR_ARTISTA <- do.call(ggarrange, c(gg_fdr_ARTISTA, ncol = 3, nrow = 2))

(AA = ggpubr::ggarrange(FDR_ARTISTA, legend,
                        widths = c(3,1),
                        ncol = 2 ))

ggsave(filename = paste0(path_fig, 'supp12.pdf'),
       plot = AA,
       device = "pdf",
       width = 12,
       height = 8,
...
library(dplyr)
library(stringr)
result_combined <- read_csv(paste0(path_data,"individual_results.csv"))
scenarios <- list(
  c("DSP1", "NULL1", "NULL2"),
  c("DSP2", "NULL1", "NULL2"),
...
gg_fdr_ARTISTA <- list()
for (s in cluster_seq) {
  sub_df <- result_combined %>% filter(nclust == s) 
...
ggsave(filename = paste0(path_fig, 'supp13.pdf'),
       plot = AA,
       device = "pdf",
       width = 12,
       height = 8,
