...
########################## LIBD global test ############################
########################################################################
path_data <- "./data/lowly_abundant/LIBD_main/"
######################### Load data #############################
...
########################################################################
path_data <- "./data/lowly_abundant/LIBD_individual/"
######################### Load data #############################
...
########################## ARTISTA global test ############################
########################################################################
path_data <- "./data/lowly_abundant/ARTISTA_main/"
######################### Load data #############################
pval <- read.csv(paste0(path_data,"pval_all.csv"), row.names = 1)
padj <- read.csv(paste0(path_data,"padj_all.csv"), row.names = 1)
gene_lists <- readRDS(paste0(path_data, "geneGT.rds"))
...
########################################################################
path_data <- "./data/lowly_abundant/ARTISTA_individual/"
######################### Load data #############################
result_combined <- read.csv(paste0(path_data,"results.csv"), row.names = 1)
...
names(colors_method) <- gsub("_Banksy", "", names(colors_method))
names(all_colours) <- gsub("_Banksy", "", names(all_colours))
...
ggsave(filename = paste0(path_fig, 'supp_main_FDR.pdf'),
       plot = AAA,
       device = "pdf",
       width = 12,
       height = 8,
