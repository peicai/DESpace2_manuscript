...
rownames(truth) <- gene
methods_order <- c("DESpace_GT","DESpace_BayesSpace")
methods_names <- c("ground truth", "recomputed clusters")
...
(gg_fdr_LIBD_1 <- plot_fdrtprcurve(cobra_plot,plottype = c("points"),
    ...
    scale_x_sqrt( breaks = c(0.00, 0.01, 0.02), limits = c(0,0.02)) +
...
(gg_fdr_LIBD_2 <- plot_fdrtprcurve(cobra_plot,plottype = c("points"),
                                   pointsize = 0, linewidth = 2)+
    scale_x_sqrt( breaks = c(0.00, 0.01, 0.02, 0.05), limits = c(0,0.05)) +
    coord_cartesian(ylim = c(0.95, 1)) +
    scale_color_manual(values = all_colours,
                       name = "",
                       breaks=methods_order,
...
truth <- data.frame(status = c(rep(1, length(scenario$gene1)), rep(0, length(scenario$gene2))))
rownames(truth) <- gene
methods_order <- c("DESpace_GT","DESpace_Banksy")
methods_names <- c("ground truth", "recomputed clusters")
# Create COBRA data and calculate performance
pval <- pval[, colnames(pval) %in% methods_order]
padj <- padj[, colnames(padj) %in% methods_order]
...
# Plot ROC and FDR/TPR curves
(gg_fdr_ARTISTA_1 <- plot_fdrtprcurve(cobra_plot,plottype = c("points"),pointsize = 0, linewidth = 2)+
    scale_x_sqrt( breaks = c(0.00, 0.01, 0.02), limits = c(0,0.02)) +
    coord_cartesian(ylim = c(0.95, 1)) +
    scale_color_manual(values = all_colours,
                       name = "",
    ...
    shape = rep(shape_fill[sel_shape],3), 
    stroke = 2, alpha = 0.25)+
    fig_theme + theme(legend.position = "bottom"))#, legend.direction = "horizontal") )
...
(gg_fdr_ARTISTA_2 <- plot_fdrtprcurve(cobra_plot,plottype = c("points"),
                                      pointsize = 0, linewidth = 2)+
    #scale_x_sqrt( breaks = c(0.00, 0.01, 0.02, 0.2), limits = c(0,0.05,0.2, 0.5, 1)) +
    scale_x_sqrt( breaks = c(0.00, 0.01, 0.05, 0.1, 0.2), limits = c(0,0.05)) +
    coord_cartesian(ylim = c(0.95, 1)) +
    scale_color_manual(values = all_colours,
                       name = "",
                       breaks=methods_order,
...
ggsave(filename = paste0(path_fig, 'supp_DESpace_FDR.pdf'),
       plot = AAA,
       device = "pdf",
       width = 12,
       height = 12,
